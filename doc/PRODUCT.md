...
- **Scoring**: Each correct placement earns `(1000 + DifficultyBonus + MasteryBonus) × SpeedMult × StreakMult`. DifficultyBonus = `2310 × (10/(gap+10))` where gap = year distance to nearest neighbor. MasteryBonus = `500 × Mastery_Acc²` where `Mastery_Acc = min(1, correct / max(correct, attempts-1))` (one-mistake grace). SpeedMult = `1 + 0.25 × (max(0,20-s)/19)²`. StreakMult tiers: 1→1.0, 2→1.05, 3→1.2, 4→1.3, 5→1.4, 6+→1.5. Completion Bonus at target = `(target/attempts) × (target×500)`. Incorrect placements earn a **Consolation** bonus: `max(1, round(75 × gapFactor × edgeFactor))` where gapFactor = `max(0,(150-gap)/150)` and edgeFactor = `max(0,(50-edgeDist)/50)` (edgeDist = distance from card's year to nearest correct-slot boundary). Floor is 1 — every miss earns at least 1 point.
