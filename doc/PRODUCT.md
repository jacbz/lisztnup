...
- **Leaderboard**: Only completed timelines can publish. Endgame celebrates new global, country, and browser-token personal records in that priority order. Endgame asks once per player name; allow/deny choices are saved in settings. Published names can be renamed later, and naming a score claims anonymous history for the same browser token. Custom tracklists publish under `custom` and stay out of Tracklist Records.
...
- **Preview**: live counts plus category, composer, and decade-bucket year distribution with hover drilldowns
