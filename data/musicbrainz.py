...
WORK_TYPES = {
    ...
    11: "Oratorio", 12: "Overture", 13: "Partita", 14: "Quartet",
    15: "Song-cycle", 16: "Symphony", 17: "Song", 18: "Symphonic poem",
    24: "Operetta", 30: "Incidental music"
}
...
JOIN musicbrainz.isrc AS i ON r.id = i.recording
INNER JOIN musicbrainz.deezer AS d ON i.isrc = d.isrc
...
WHERE lrw.entity1 = %(work_id)s ORDER BY r.name;
"""
...
def get_work_details_recursive(cursor, work_id, label_counter):
    ...
    recordings = [{"gid": rec['recording_gid'], "name": rec['recording_name'], "isrc": rec['isrc'],
                   "label": rec['recording_label'], "deezerId": rec['deezer_id']} for rec in recordings_data]
    for rec in recordings:
        if rec['label']: label_counter.update([rec['label']])
    ...
    for normalized_name, duplicates in grouped_subworks.items():
        ...
        if len(duplicates) > 1:
            ...
            for subwork_row in duplicates:
                sub, recs, count, types = get_work_details_recursive(cursor, subwork_row['work_id'], label_counter)
                if count > max_recs:
                    max_recs = count
                    winner_row = subwork_row
                    best_subwork_details = (sub, recs, count, types)
            child_subworks, child_recordings, child_rec_count, child_descendant_types = best_subwork_details
        else:
            child_subworks, child_recordings, child_rec_count, child_descendant_types = get_work_details_recursive(cursor, winner_row['work_id'], label_counter)
        ...
        if child_rec_count > 0:
            ...
            valid_subworks.append({
                "gid": winner_row['work_gid'], "name": winner_row['alias_name'] or winner_row['original_name'],
                "begin_year": winner_row['work_begin_year'], "end_year": winner_row['work_end_year'],
                "recordings": child_recordings, "subworks": child_subworks
            })
...
def print_statistics(final_data, stats):
    ...
    top_composers = sorted(final_data, key=lambda c: len(c['works']), reverse=True)[:200]
    ...
    top_works = sorted(stats['works_by_recording_count'], key=lambda x: x[2], reverse=True)[:20]
    ...
    work_type_counts = sorted(stats['work_type_counts'].items(), key=lambda x: x[1], reverse=True)
    ...
    buckets = [(1, 1), (2, 5), (6, 10), (11, 25), (26, 50), (51, 100), (101, 250), (251, float('inf'))]
...
def main():
    conn = None
    composers = {}
    stats = { 'total_recordings': 0, 'work_type_counts': defaultdict(int), 'work_type_recording_sum': defaultdict(int), 
              'recordings_per_work_dist': [], 'works_by_recording_count': [], 'label_counter': Counter() }
    ...
    try:
        ...
        for work in top_level_works_raw:
            grouped_works[(work['composer_id'], normalize_name(work['original_name']))].append(work)

        print(f"Found {len(top_level_works_raw)} candidate works, grouped into {len(grouped_works)} unique top-level works.")
        ...
        for (composer_id, normalized_name), duplicates in grouped_works.items():
            ...
            descendant_types = []
            if len(duplicates) > 1:
                best_work_details = None; max_recs = -1
                for work_row in duplicates:
                    sub, recs, count, types = get_work_details_recursive(cursor, work_row['work_id'], stats['label_counter'])
                    if count > max_recs:
                        max_recs = count; winner_row = work_row
                        best_work_details = (sub, recs, count, types)
                subworks, recordings, total_recordings, descendant_types = best_work_details
            else:
                subworks, recordings, total_recordings, descendant_types = get_work_details_recursive(cursor, winner_row['work_id'], stats['label_counter'])

            if total_recordings > 0:
                composer_name = winner_row['composer_sort_name']
                final_work_name = winner_row['alias_name'] or winner_row['original_name']
                ...
                if composer_id not in composers:
                    composers[composer_id] = {"gid": winner_row['composer_gid'], "name": composer_name, "birth_year": winner_row['composer_birth_year'],
                                              "death_year": winner_row['composer_death_year'], "works": []}
                
                work_obj = {"gid": winner_row['work_gid'], "name": final_work_name, "type": work_type_str,
                            "begin_year": winner_row['work_begin_year'], "end_year": winner_row['work_end_year'],
                ...
                stats['works_by_recording_count'].append((final_work_name, composer_name, total_recordings))
                
                truncated_name = (final_work_name[:57] + '...') if len(final_work_name) > 60 else final_work_name
                print(f"{composer_name:<30}\t{truncated_name:<60}\t{total_recordings}")

        final_data = sorted([c for c in composers.values() if c['works']], key=lambda c: c['name'])
